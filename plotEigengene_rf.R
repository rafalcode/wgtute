...
plotEigengeneNetworks_rf = function(
                      multiME, setLabels,
  ...
  if (plotHeatmaps) for (i.row in (1:nSets)) for (i.col in (1:nSets))
  {
    ...
    } else
    {
      ...
      if (i.row>i.col)
      {
        ...
        # here is the RF edit, just this one line
        # the idea is that we want all the 1-abs(dispd) values comig between the lower and upper zlimPreser limits.
        # but the test must be implemented correctly ...
        if ( sum( (1-abs(dispd))<zlimPreservation[1]) || sum((1-abs(dispd))>zlimPreservation[2]) )
